...
load_dotenv()
ERNIE_CLIENT = OpenAI(
    api_key="850b0841ea3ccaac9ee38c4659a157b769adc267",  # 替换为你的 ERNIE API Key
    base_url="https://aistudio.baidu.com/llm/lmapi/v3",  # ERNIE API 地址
)
...
class TaskController(Node):
    ...
    def task_sequence(self):
        ...
        try:
            execute_movement(self.cmd_pub, "forward", 103.0)
            # 第一阶段：后退移动
            logger.info("阶段1: 后退移动")
            ...
            execute_movement(self.cmd_pub, "forward", 7)
            
            # 第三阶段：根据指令转向
            direction = interpret_direction(ocr_text, logger)
            logger.info(f"解析方向: {direction}")
            if direction == "left":
                logger.info(f"阶段3: 向{direction}移动")
                execute_movement(self.cmd_pub, direction, 5.5)
                execute_movement(self.cmd_pub, "forward", 4)
                execute_movement(self.cmd_pub, "right", 5.5)
                # 第四阶段：再次向前移动
                logger.info("阶段4: 向前移动")
                execute_movement(self.cmd_pub, "forward", 6)
                execute_movement(self.cmd_pub, "right", 5.5)
                execute_movement(self.cmd_pub, "forward",4 )
                execute_movement(self.cmd_pub, "left", 5.5)
            else:
                logger.info(f"阶段3: 向{direction}移动")
                execute_movement(self.cmd_pub, direction, 5.5)
                execute_movement(self.cmd_pub, "forward", 4)
