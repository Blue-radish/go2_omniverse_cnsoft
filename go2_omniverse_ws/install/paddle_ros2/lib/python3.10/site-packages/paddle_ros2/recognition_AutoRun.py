#!/usr/bin/env python3
"""
go2_visual_servoing.py

Unitree Go2 visual servoing controller:
1. 订阅相机图像并进行目标检测
...
# ───────── 视觉伺服控制节点 ─────────────────────────────────────────
class Go2VisualServoing(Node):
    def __init__(self):
        super().__init__("go2_visual_servoing")
        ...
        self.target_class = "ball"     # 目标类别名称 (根据模型标签修改)
        ...
        try:
            ...
            if self.target_class_id is None:
                ...
                if self.target_class_id is None:
                    ...
                    self.get_logger().warn(f"Target class '{self.target_class}' not found. Using first class.")
            
            self.get_logger().info(f"Model loaded. Target class: {self.pred_config.labels[self.target_class_id]}")
        ...
        self.get_logger().info("Visual servoing controller ready. Waiting for images...")
    ...
    def control_robot(self, target_center, image_width, image_height):
        ...
        if len(self.detection_history) == 3 and not any(self.detection_history):
            self.get_logger().info("Target lost for 3 consecutive frames. Stopping.")
        ...
        if target_center is None:
            self.get_logger().info("Target not detected. Stopping.")
        ...
        if abs(normalized_offset) < self.stop_threshold:
            ...
            self.get_logger().info(f"Target centered. Moving forward at {self.move_forward_speed:.2f} m/s")
        else:
            self.get_logger().info(f"Adjusting rotation: {rotation_speed:.3f} rad/s (offset: {offset_x:.1f} px)")
