...
def main(args=None):
    ...
    try:
        ...
        # 根据OCR结果执行不同的越障动作
        if direction == "right":
            navigator.waypoints.append((0.94, -0.4 ,0.0)) # 目标点绝对坐标
            navigator.update_parameters(movement_axis_order='xy',velocity=0.5)
            navigator.navigate()
            ...
            navigator.update_parameters(movement_axis_order='xy',velocity=0.6, timeout=15)
            navigator.navigate()
        ...
        navigator.update_parameters(velocity=0.7, movement_axis_order='xy',rotation_velocity=0.5) # 更新参数 先x调整后y调整
        ...
        navigator.update_parameters(velocity=0.7, movement_axis_order='xy',rotation_velocity=0.5,timeout=15)
        navigator.navigate()
