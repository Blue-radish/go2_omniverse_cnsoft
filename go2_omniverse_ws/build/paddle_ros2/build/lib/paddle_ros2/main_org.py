...
def main(args=None):
    ...
    try:
        ...
        tracker.update_parameters(obstacle_threshold=0.55,forward_speed=0.7)
        ...
        if direction == "right":
            ...
            time.sleep(1.0)
            navigator.waypoints.append((1.80, -0.4 ,0.0)) # 相对坐标的方式给出运动
            navigator.navigate()
        ...
        navigator.waypoints.append((1.5, 1.6 ,math.pi)) # 绝对坐标方式给出目标点位
        ...
        navigator.clear_waypoints()
        navigator.waypoints.append((1.5, 2.6, 0.0)) # 绝对坐标方式给出目标点位
        navigator.update_parameters(velocity=0.7, movement_axis_order='xy',rotation_velocity=0.5)
        navigator.navigate()
        demo.print_position("前往终点-1")  # 打印位置
        time.sleep(1.0)

        navigator.waypoints.append((3.0, 2.6, 0.0)) # 绝对坐标方式给出目标点位
        navigator.update_parameters(velocity=0.5, rotation_velocity=0.5)
        navigator.navigate()
